...
import sacremoses
tok=sacremoses.MosesTokenizer(lang='cs')
detok=sacremoses.MosesDetokenizer(lang='cs')
...
#            "ref": ref
    ...
    seg_scores, sys_score = model_qe.predict(data, 128, gpus=1, progress_bar=False)
    return seg_scores
...
#                print(c)
        ...
        if gen % 10 == 0:
            ...
            logging.warning("pop sample: {}".format('\n'.join(' '.join(p) for p in pop[:250])))

    return [best, best_eval]

def ga_init(sourcef, transf, reff, dictf):
    ...
    with open(sourcef) as srcs, open(transf) as trans, open(reff) as refs, open(dictf) as dict_tgts:
        ...
        for dict_tgt, src, ref in zip(dict_tgts, srcs, refs):
          ...
          #  possible_tgt = list(set([t for sent in init_pop for t in sent])) + [''] + dict_tgt.strip().split(' ')
            ...
            out = genetic_algorithm(fitness_comet_mbr_and_qe, pop, max_len, 400, len(pop), 0.07, 0.07, src, ref, possible_tgt,src_embeddings=src_embeddings,init_mt_embeddings=init_mt_embeddings)[0]
...
def mbr_command() -> None:
    ...
    ga_init(cfg.sources(),cfg.translations(), cfg.ref(), cfg.dict())
    exit()
