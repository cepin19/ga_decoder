    ...
    for i in range(len(bitstring)):
        ...
        if rand() < r_mut:
            ...
            if l>1:
                ...
                space_i=[i for i, x in enumerate(bitstring) if x == '']
                if l>len(space_i):#wait, thats illegal (we cant make the gene longer)
                    continue
                for x in range(l):
                    del bitstring[space_i[-x]]  #we need to iterate backwards to not mess up the order

                bistring[i]=tgt[0]
                for x in range(1,l):
                    bistring.insert(i+x,tgt[x])
                logging.waring("inserting {} instead of {}".format(tgt, bistring[i]))
            else:
                x=0
                bitstring[i] = tgt[x]
...
f="news19.en.snt_trans_n20_head100"
with open("{}.trans".format(f)) as trans, open("{}.scores".format(f)) as scores, open("{}.ref".format(f)) as refs, open("{}.src".format(f)) as srcs, open("{}.tgt_words_not_exp".format(f)) as dict_tgts:
    ...
    for dict_tgt,src,ref in zip(dict_tgts,srcs,refs):
        ...
        possible_tgt=list(set([tok for sent in init_pop for tok in sent]))+['']+dict_tgt.strip().split(' ')
        max_len=int(max([len(s) for s in pop])*1.0)
        # PAD
        pop=[(p + max_len*[''])[:max_len] for p in pop]*200
        #print(pop)
#        print(pop
        out=genetic_algorithm(fitness_comet_multi,pop,max_len,500,len(pop),0.05,0.05,src,ref)[0]
